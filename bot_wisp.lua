		...
		elseif( tablelength( creepsEnemy ) == 0 )then
			...
			for iEnemy,creepEnemy in pairs(creepsEnemy) do
				...
				if( hpCreepEnemy  < baseDmageHero + 150 )then
					...
					-- lastCreep(creepEnemy,creepsAlly,towersAlly);
...
function lastCreep(creepEnemy,creepsAlly,towersAlly)
	...
	if(timeAttack[creepEnemy] == nil)then
		...
		if(nextAttackAlly[creepAlly] ~= nil)then
			...
			while(true)do
				...
				if( hpCreepEnemy - damageFromAlly <  damageFromHero )then
					timeAttack[creepEnemy] = timeMin - 1;
					break;
				else
		...
		timeNow = DotaTime();
		if( (timeAttack[creepEnemy] - 0.2) < timeNow and timenow < (timeAttack[creepEnemy] + 0.2)  )then
			npcBot:Action_AttackUnit( creepEnemy, true );
