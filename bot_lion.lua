...
function lastCreep(creepEnemy,creepsAlly,towersAlly)

	hpCreepEnemy = creepEnemy:GetHealth();
	-- print(creepEnemy)
	-- print("--")
	-- printTable2(timeAttack)
	-- print("++")
	if(timeAttack[creepEnemy] == nil)then

		nextAttackAlly = {}

		for iAlly,creepAlly in pairs(creepsAlly) do
			if( creepAlly:GetAttackTarget() == creepEnemy )then					
				nextAttackAlly[creepAlly] = creepAlly:GetLastAttackTime();
			end
		...
		for iAlly,towerAlly in pairs(towersAlly) do
			if( towerAlly:GetAttackTarget() == creepEnemy )then
				nextAttackAlly[towerAlly] = towerAlly:GetLastAttackTime();
			end

		end

		print(tablelength(nextAttackAlly))

		if(nextAttackAlly ~= nil)then

			i = 0
			while(i<100)do
				-- print("in loop"..i)
				creepAllyMin,timeMin = findMin(nextAttackAlly);

				-- printTable2(nextAttackAlly)
				-- print("timeMin :"..timeMin)
								
				-- print("Timenow :"..GameTime())
				if(creepAllyMin ~= nil)then		

					damageFromAlly = creepEnemy:GetActualIncomingDamage( creepAllyMin:GetAttackDamage() , DAMAGE_TYPE_PHYSICAL )
					damageFromHero= creepEnemy:GetActualIncomingDamage( npcBot:GetAttackDamage() , DAMAGE_TYPE_PHYSICAL )
					if( hpCreepEnemy - damageFromAlly < 0 )then
						print("creep die")

						timeAttack[creepEnemy] = timeMin - 0.5;
						break;
					elseif( hpCreepEnemy - damageFromAlly <  damageFromHero - 20 )then
						
						timeAttack[creepEnemy] = timeMin;
						print("++++++++++++++++++")
						print(creepEnemy)

						print("time attack"..timeAttack[creepEnemy])
						break;
					else
						nextAttackAlly[creepAllyMin] = nextAttackAlly[creepAllyMin] + creepAllyMin:GetSecondsPerAttack();
					end
					i = i+ 1;
					hpCreepEnemy = hpCreepEnemy - damageFromAlly;
					print("HP :"..hpCreepEnemy)
				end
		...
		-- if(timeNow > timeAttack[creepEnemy] + 0.2 )then -- more than delete creep
		-- 	timeAttack[creepEnemy] = nil;
		-- 	print("creep delete ")
		-- else
		if(timeAttack[creepEnemy] == 0)then


		
		elseif( timeAttack[creepEnemy] < timeNow  )then -- and timenow < timeAttack[creepEnemy] + 0.2 
			--if( timenow < timeAttack[creepEnemy] + 0.2 )then

				print("----------")
				-- print(creepEnemy)
				-- print("Timenow :"..timeNow)
				print(timeAttack[creepEnemy])

				npcBot:Action_AttackUnit( creepEnemy, true );
				print("attack ")
				timeAttack[creepEnemy] = 0;
			--end
		end
